...
!!! Warning
    Always take a backup of the Postgres database before upgrading patra

## Binary
- Stop the running instance of patra.
- Download the [latest release](https://github.com/appzenowebservices/Omnipost/releases) and extract the patra binary and overwrite the previous version.
- `./patra --upgrade` to upgrade an existing database schema. Upgrades are idempotent and running them multiple times have no side effects.
- Run `./patra` again.

If you installed patra as a service, you will need to stop it before overwriting the binary. Something like `sudo systemctl stop patra` or `sudo service patra stop` should work. Then overwrite the binary with the new version, then run `./patra --upgrade, and `start` it back with the same commands.

If it's not running as a service, `pkill -9 patra` will stop the patra process.

## Docker
**Important:** The following instructions are for the new [docker-compose.yml](https://github.com/appzenowebservices/Omnipost/blob/master/docker-compose.yml) file.
...
```shell
docker-compose down
docker-compose pull && docker-compose run --rm app ./patra --upgrade
...
3. If you're using `docker compose`, edit `docker-compose.yml` and change `patra:latest` to `patra:v2.4.0` _(for example)_.
4. Restart.
...
1. Stop patra (app):
```
sudo docker stop patra_app
```
2. Restore your pre-upgrade db (required) _(be careful, this will wipe your existing DB)_:
```
psql -h 127.0.0.1 -p 9432 -U patra
drop schema public cascade;
create schema public;
\q
psql -h 127.0.0.1 -p 9432 -U patra -W patra < patra-preupgrade-db.sql
```
3. Edit the `docker-compose.yml`:
```
x-app-defaults: &app-defaults
  restart: unless-stopped
  image: patra/patra:v2.4.0
```
4. Restart:
