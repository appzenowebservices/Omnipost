...
Patra supports single sign-on with OIDC (OpenID Connect). Any standards compliant OIDC provider can be configured in Settings -> Security -> OIDC
...
Tutorials for configuring patra SSO with popular OIDC providers.

## Keycloak
Keycloak configuration for patra SSO integration.
...
- **General Settings**
    ...
    - **Client ID**: `patra` (or any preferred name)
    - **Name**: Optional descriptive name (e.g., "patra SSO")
- **Capability Config**:
...
- **Login Settings**:
    - **Root URL**: Copy the **Redirect URL for oAuth provider** value from patra Admin -> Settings -> Security -> OIDC. It will look like `https://patra.yoursite.com/auth/oidc`
...
### 2. Configure Patra
2. In Patra Admin -> Settings -> Security -> OIDC.
    ...
    - **Provider name**: Set a name to show on the patra login form, eg: `Login with OrgName`
    - **Client ID**: Client ID set in Keycloak, eg: `patra`
...
## Authentik  
Authentik configuration for patra SSO integration.

### 1. Create a new OIDC provider in Authentik
In the Authentik admin interface, create a new OIDC provider for patra.

- **Provider Settings**:  
    - **Name**: `patra` (or any preferred name)
    - **Signing Key**: `authentik Self-signed Certificate`
    - **Client Type**: `Confidential`
    - **Client ID**: `patra` (or any preferred name)
    - **Redirect URIs**: Copy the **Redirect URL for oAuth provider** value from patra Admin -> Settings -> Security -> OIDC. It will look like `https://patra.yoursite.com/auth/oidc`
...
- **Application Settings**:
    - **Name**: `patra` (or any preferred name)
    - **Slug**: `patra` (or any preferred slug. Used in the redirect URL)
    - **Provider**: Select the OIDC provider created in the previous step

### 3. Configure patra
In patra Admin → Settings → Security → OIDC:
...
- **Client ID**: Client ID set in Authentik (e.g., `patra`)
- **Client Secret**: Client Secret copied from Authentik
- **Auto-create users from SSO**: (Optional) Enable to automatically create users who don't exist
- **Default user role**: (Required if auto-create enabled) Select role for new users

## Google Workspace  
Google Workspace (Google Cloud) configuration for patra SSO integration.
...
- **Project Settings**:  
    - **Project name**: `Patra` (or any preferred name)
- **Branding Settings**:
    - **App name**: `Patra` (or any preferred name, this will be visible to the users.)
    - **Authorised domains**: `patra.example.com` (or domains that your instance is available on.)
...
- **Application Settings**:
    ...
    - **Name**: `patra` (or any preferred name)
    - **Authorised JavaScript origins**: `https://patra.example.com` (or domains that your instance is available on.)
    - **Authorised redirect URIs**: `https://patra.example.com/auth/oidc` (or domains that your instance is available on, value is also available in the Settings mentioned above. (Redirect URL for oAuth provider))
