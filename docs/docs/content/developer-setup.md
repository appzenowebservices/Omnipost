...
### First time setup
`git clone https://github.com/appzenowebservices/Omnipost.git`. The project uses go.mod, so it's best to clone it outside the Go src path.

1. Copy `config.toml.sample` as `config.toml` and add your config.
2. `make dist` to build the patra binary. Once the binary is built, run `./patra --install` to run the DB setup. For subsequent dev runs, use `make run`.
...
1. Locally
    ...
    - Run `make run` to start the patra dev server on `:9000`.
    - Run `make run-frontend` to start the Vue frontend in dev mode using yarn on `:8080`. All `/api/*` calls are proxied to the app running on `:9000`. Refer to the [frontend README](https://github.com/appzenowebservices/Omnipost/blob/master/frontend/README.md) for an overview on how the frontend is structured.
...
# Production build
Run `make dist` to build the Go binary, build the Javascript frontend, and embed the static assets producing a single self-contained binary, `patra`
